...
<div class="form-group">
  <?php if (($form_element->show_title and $form_element->label_position == "top") or (!$form_element->show_title)) { ?>
    ...
    <?php foreach($form_element->form_element_option as $form_element_option) { ?>
      <input type="checkbox" class="flat"  name="iCheck" <?=(isset($form_element_option->option_value) and $form_element->default_value == $form_element_option->option_value) ? "checked" : ""?>> <?php echo $form_element_option->option_value; ?>
    <?php } ?>
  <?php } else { ?>
    <?php if ($form_element->show_title) { ?>
