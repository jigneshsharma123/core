<form name="frmnewdropdown" method="post" action="<?php echo base_url(); ?>admin/custom_forms/<?php echo (isset($form_element) and isset($form_element->id)) ? 'updateelement' : 'createnewelement' ?>">
  ...
  <div class="form-group">
    <div id="options">
      <div>
        <label for="option"> <a href="javascript:void(0)" id="add_options">Add Options +</a></label>
      </div>
      <?php if (isset($form_element)) { ?>
        ...
        <?php foreach($form_element->form_element_option as $form_element_option) { ?>
          <div class="input-group"><input class="form-control" name="options[]" type="text" value="<?php echo (isset($form_element_option->option_value)) ? $form_element_option->option_value : '' ?>"><div class="input-group-addon" id="remove_component" style="cursor:pointer"><i class="fa fa-remove"></i></div></div>          
        <?php } ?>
      <?php } ?>
    </div>
  </div>  
...
<script>
  $("#add_options").click(function(){
      $("#options").append('<div class="input-group"><input class="form-control" name="options[]" type="text"><div class="input-group-addon" id="remove_component" style="cursor:pointer"><i class="fa fa-remove"></i></div></div>')
  })
  
  $( "#options" ).on( "click", "#remove_component", function() {
    $( this ).parent().remove();
  });
</script>
